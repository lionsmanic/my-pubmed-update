...
# --- 頁面設定 ---
st.set_page_config(page_title="GynOnc 文獻快篩系統 v4.1", page_icon="⚡", layout="wide")

# --- Session State 初始化 ---
if 'articles_data' not in st.session_state:
    st.session_state.articles_data = []  # 存搜尋到的所有文章簡介
if 'analysis_cache' not in st.session_state:
    st.session_state.analysis_cache = {} # 存已經分析過的詳細內容 {id: html}
...
with st.sidebar:
    ...
    if st.button("🚀 極速搜尋 (列出標題)", type="primary", disabled=(not selected_model_name)):
        ...
        st.session_state.search_trigger = True # 觸發搜尋
...
def run_deep_analysis(art, key, model_name):
    ...
    4. 所有內容必須包在一個最外層的 <div style="font-family: sans-serif;"> 標籤內。
...
def send_bulk_email(to, pwd, queue):
    ...
    <h2 style="color: #2c3e50;">🧬 您的精選文獻分析報告</h2>
    <p style="color: #666;">本郵件由 AI 助手彙整發送，包含您挑選的深度分析內容。</p>
    <hr>
    """
    for item in queue:
        body += item['html']
...
st.caption("v4.1 修復版：解決顯示亂碼，優化 HTML 輸出")
...
if st.session_state.articles_data:
    ...
    for i, art in enumerate(st.session_state.articles_data):
        with st.container():
            ...
            with col2:
                btn_key = f"analyze_btn_{art['id']}_{i}"
                if st.button("🔍 詳細分析", key=btn_key):
                    with st.spinner("AI 正在深度閱讀..."):
                        if art['id'] not in st.session_state.analysis_cache:
                            ...
                            email_item = {
                                ...
                                <div style="background-color: #f9f9f9; padding: 20px; border-radius: 5px; margin-bottom: 20px;">
                                    ...
                                    <h4 style="margin-top: 5px; color: #2e86c1;">{art['title_zh']}</h4>
